...
export default function Navbar() {
  ...
  return (
    <nav className="w-full bg-transparent transition-all duration-300">
              ...
              <div className="hidden lg:flex lg:items-center lg:gap-8">
                {navItems.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`hover:text-readly-blue text-[15px] font-semibold transition-all ${pathname === item.href
      ...
      <div
        className={`fixed inset-0 z-40 bg-zinc-900/50 backdrop-blur-sm transition-opacity duration-300 lg:hidden ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"
          }`}
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 w-72 transform bg-white shadow-2xl transition-transform duration-300 ease-in-out lg:hidden ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        ...
        <div className="flex flex-col py-6">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              onClick={() => setIsOpen(false)}
              className={`px-8 py-4 text-lg font-bold transition-all hover:bg-zinc-50 hover:pl-10 ${pathname === item.href
                ? "text-readly-blue"
